...
export default function TesellumAnaliz() {
  ...
  const renderDurumBadge = (durum: TesellumItem["durum"]) => {
    switch (durum) {
      case "aleyhte_fark": return <span className="flex items-center gap-1 bg-red-100 text-red-700 px-2 py-1 rounded-md text-xs font-bold"><TrendingUp size={14} /> ZAMLI</span>;
      case "lehte_fark": return <span className="flex items-center gap-1 bg-green-100 text-green-700 px-2 py-1 rounded-md text-xs font-bold"><TrendingDown size={14} /> UCUZLAMIŞ</span>;
      case "fark_yok": return <span className="flex items-center gap-1 bg-slate-100 text-slate-600 px-2 py-1 rounded-md text-xs font-bold"><CheckCircle2 size={14} /> AYNI FİYAT</span>;
      case "yeni_urun": return <span className="flex items-center gap-1 bg-blue-100 text-blue-700 px-2 py-1 rounded-md text-xs font-bold"><HelpCircle size={14} /> YENİ ÜRÜN</span>;
      case "eksik_veri": return <span className="flex items-center gap-1 bg-orange-100 text-orange-700 px-2 py-1 rounded-md text-xs font-bold"><AlertTriangle size={14} /> DİKKAT</span>;
      default: return <span className="bg-gray-100 text-gray-500 px-2 py-1 rounded-md text-xs font-bold">BİLİNMİYOR</span>;
    }
  };

  // YÜKLEME KONTROLÜ BURADA DÜZELTİLDİ
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    const selectedFile = e.target.files?.[0];
    console.log("Seçilen dosya:", selectedFile?.name); // Konsola dosya adını basacak
    ...
    <main className="min-h-screen bg-slate-50 p-8">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
          ...
          <p className="text-slate-500 mb-6 text-sm">ElektraWeb Excel fişini yükleyin. Hiçbir veri gizlenmeyecek, kontrol daima sizde olacak.</p>
